...
{
    public class AboutContentDataAccess : BaseDataAccess
    {
        public List<MasterAboutContent> GetHomeContent()
        {
            ...
            try
            {
                ...
                obj = AdoHelper.ExecuteReader<MasterAboutContent>(CommandType.Text, "select CONTENT_ID as ContentID,CONTENT_NAME as ContentName from VS_CONTENT_MST where delete_flag='0' and status_id=4  and CONTENT_ID in(21) order by ContentID", sqlParameters);
            }
            catch (Exception ex)
            {
