...
namespace Vastu.Controllers
{
    ...
    {
        private HomeContentBusinessLayer _HomeContentBusinessLayer;
        public HomeContentController()
        {

            _HomeContentBusinessLayer = new HomeContentBusinessLayer();
        }
        public ActionResult Index()
        {
            ...
            try
            {
                obj= _HomeContentBusinessLayer.GetHomeContent();
                obj.Insert(0, new MasterContent { ContentID = 0, ContentName = Vastu.Resources.Resource.Select  });
                ViewBag.Home = obj;
            }
            catch (Exception ex)
            {
        ...
        public JsonResult LoadSelectedContent(string ContentID)
        {
            List<MstrContent> obj = new List< MstrContent>();
            List<ShowHomeContent> obj1 = new List<ShowHomeContent>();
            

            try
            {
                obj= _HomeContentBusinessLayer.LoadDescription(Convert.ToInt32(ContentID));
        ...
        public JsonResult SavePageInformation(SaveHomeContent savecontent)
        {
            int result = 0;
            try
            {
                SaveContent cont = new SaveContent();
                cont.ContentID = savecontent.ContentID;
                cont.ContentDescription = savecontent.ContentDescription;
                result=  _HomeContentBusinessLayer.SaveContentInformation(cont);
        ...
        public JsonResult DeleteContentById(string ContentID)
        {
            ...
            try
            {               
                result = _HomeContentBusinessLayer.DeleteContentById(Convert.ToInt32 ( ContentID));
