...
namespace MujiStore.Controllers
{
    ...
    public class VideoDeployStatusController : BaseController
    {
        ...
        public ActionResult Index(string sortOrder, int? page, string SearTitle, string SearchTitle, string SearchFromCRTDT, string CdStDate, string SearchToCRTDT, string CdEdDate, int? ConvertStatus, int? CvtStatus, int? FolderID, int? FolID)
        {
            ...
            lstConvertStatus.Add(new SelectListItem { Text = MujiStore.Resources.Resource.Deployed, Value = "3" });//VJ 20200603 ConvertStatus Changed
            lstConvertStatus.Add(new SelectListItem { Text = MujiStore.Resources.Resource.NotDeployed, Value = "0" });
            ...
            try
            {
                ...
                query = query + " (select tblMedia.MediaID,Title,Description,Video,ConvertStatus,tblMedia.FolderID,Name,ApprovalStatus ";
                query = query + " ,case ds.IsExists when 1 then 3 else 0 end [deploystatus] from tblmedia ";
                query = query + " left join(select distinct MediaID,IsExists from tblDeployStatus where IsExists = 1 and DELFG = 0 )ds  on DS.MediaID = tblMedia.MediaID ";
                query = query + " left join tblFolder on tblMedia.FolderID = tblFolder.FolderID ";
                query = query + " where tblmedia.DELFG = 0 and FORMAT(tblMedia.CRTDT, 'yyyy-MM-dd') between '" + CdStDate + "' and '" + CdEdDate + "') select * from cte where mediaid is not null";
                ...
                if (ViewBag.SearTitle != null && ViewBag.SearTitle != "" && ViewBag.SearTitle != string.Empty)
                    ...
                    {
                        query = query + " and deploystatus = " + ViewBag.cvtStatus;
                    }
                    if (ViewBag.FolID != null && ViewBag.FolID != 0)
                    {
                        query = query + " and cte.FolderID = " + Convert.ToInt32(ViewBag.FolID);
                    }
                        ...
                        using (SqlDataReader sdr = cmd.ExecuteReader())
                        {
                            while (sdr.Read())
                            {
                                videoList.Add(new tblMedia
                                {
                                    ...
                                    ApprovalStatus = Convert.ToInt32(sdr["deploystatus"])
                                });
                            }
                        }
                        con.Close();
        ...
        public ActionResult Edit(int? id)
        {
            ...
            try
            {
                ...
                //int fid = 0;
                tblVideoDemo = db.tblMedias.Where(x => x.MediaID == id).SingleOrDefault();
                // fid = tblVideoDemo.FolderID;


                using (SqlConnection con = new SqlConnection(CS))
                {
                    string query = "select tblMedia.MediaID,Title,Description,Video,ConvertStatus,tblMedia.FolderID,Name,ApprovalStatus ";
                    query = query + " ,case ds.IsExists when 1 then 3 else 0 end [deploystatus] from tblmedia ";
                    query = query + " left join(select distinct MediaID,IsExists from tblDeployStatus where IsExists = 1 and DELFG = 0 )ds  on DS.MediaID = tblMedia.MediaID ";
                    query = query + " left join tblFolder on tblMedia.FolderID = tblFolder.FolderID ";
                    query = query + " where tblMedia.MediaID = @MediaID";
                    ...
                    while (rdrs.Read())
                    {
                        ...
                        tblVideoDemo.ApprovalStatus = Convert.ToInt32(rdrs["deploystatus"]);
        ...
        public IEnumerable<tblDeployStatu> GetDeployStatus(int? id)
        {
            ...
            using (SqlConnection con = new SqlConnection(CS))
            {
                ...
                querySDtnt += "from  ( ";
                querySDtnt += " 	select SSServer SSServer from tblStreamServer where DELFG = 0";
                querySDtnt += " 	union ";
                querySDtnt += "    select SSSServer SSServer from tblStreamServerSubnet where DELFG = 0";
                querySDtnt += ") SServer ";
                ...
                while (rdrs.Read())
                {
                    ...
                    depstatus.Add(new tblDeployStatu
                    {
                        ...
                        Result = Convert.ToInt32(rdrs["Result"]),
        ...
        public ActionResult SaveVideoDetails(tblMedia _tblVideoDemo)
        {
            ...
            try
            {
                ...
                TempData["SuccMsg"] = MujiStore.Resources.Resource.CntVideoDemoSaveSaveVideoDetailsSuccMsg3;
        ...
        public int UpdateDeployStatusByMediaID(int? MediaID)
        {
            int result = 0;
            using (SqlConnection con = new SqlConnection(CS))
            {
                ...
                try
                {
                    ...
                    query = "Update tblDeployStatus set ";
                    query += "DELFG=1,UPDDT=@UPDDT,UPDCD=@UPDCD,UserIPAddress=@UserIPAddress";
