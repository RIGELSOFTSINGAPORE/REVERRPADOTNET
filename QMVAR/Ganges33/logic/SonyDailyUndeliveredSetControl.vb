...
Public Class SonydailyUndeliveredSetcontroll
    Public Function AddSonydailyUndeliveredSetcontroll(ByVal csvData()() As String, queryParams As SonyDailyUndeliveredSetModel) As Boolean
        ...
        For i = 0 To csvData.Length - 1
            If i > 0 Then '0  Header
                ...
                strCommaRepl = Trim(csvData(i)(36))
    ...
    Public Function SelectDailyUnRepairsetControll(ByVal queryParams As SonyDailyUndeliveredSetModel) As DataTable
        ...
        sqlStr = sqlStr & "COUNTRY as 'COUNTRY', REGION as 'REGION', REGION_NAME as 'REGION_NAME', ASCCODE as 'ASCCODE', ORG_NAME as 'ORG_NAME', SERVICE_SHEET_NO as 'SERVICE_SHEET_NO', CUSTOMGRP_NAME as 'CUSTOMGRP_NAME', WARRANTY_TYPE_NAME as 'WARRANTY_TYPE_NAME', WARRANTY_CATEGORY_NAME as 'WARRANTY_CATEGORY_NAME', SERVICE_TYPE as 'SERVICE_TYPE', PRODUCT_CAT as 'PRODUCT_CAT', SUB_CAT as 'PRODUCT SUB_CAT', MODEL_CODE as 'MODEL_CODE', MODEL_NAME as 'MODEL_NAME', SERIAL_NO as 'SERIAL_NO', CREATE_DATE as 'CREATE_DATE', REPAIR_COMPLETE_DATE as 'REPAIR_COMPLETE_DATE', PENDING_DELIVERY_AGE as 'PENDING_DELIVERY_AGE', PART_FEE as 'PART_FEE', LABOUR_FEE as 'LABOUR_FEE', INSPECTION_FEE as 'INSPECTION_FEE', HANDLING_FEE as 'HANDLING_FEE', TRANSPORT_FEE as 'TRANSPORT_FEE', HOMESERVICE_FEE as 'HOMESERVICE_FEE', LONGDISTANCE_FEE as 'LONGDISTANCE_FEE', TRAVELALLOWANCE_FEE as 'TRAVELALLOWANCE_FEE', DA_FEE as 'DA_FEE', DEMO_CHARGE as 'DEMO_CHARGE', INSTALLATION_FEE as 'INSTALLATION_FEE', ECALL_CHARGE as 'ECALL_CHARGE', COMBAT_FEE as 'COMBAT_FEE', ACCOUNT_RECEIVABLE as 'ACCOUNT_RECEIVABLE', CUSTOM_PAY as 'CUSTOM_PAY', SONY_PAY as 'SONY_PAY', ASC_PAY as 'ASC_PAY', CUSTOMER_NAME as 'CUSTOMER_NAME', ADDRESS as 'ADDRESS', CITY_NAME as 'CITY_NAME', PHONE as 'PHONE', MOBILE as 'MOBILE', FAX as 'FAX', EMAIL as 'EMAIL', REPAIR_MAN_NAME as 'REPAIR_MAN_NAME', REPAIR_STATUS as 'REPAIR_STATUS', LAST_CONTACT_UPDATE_DATE as 'LAST_CONTACT_UPDATE_DATE', LAST_CONTACT_UPDATE_CONTENTS as 'LAST_CONTACT_UPDATE_CONTENTS', BIN_LOCATION as 'BIN_LOCATION', POST_CODE as 'POST_CODE', St_type as 'St_type', Model_6D as 'Model 6D', convertToJob_in_MAPP as 'convertToJob_in_MAPP', completed_in_MAPP as 'completed_in_MAPP', deliver_in_MAPPCRLF as 'deliver_in_MAPPCRLF' "
        sqlStr = sqlStr & "FROM "
