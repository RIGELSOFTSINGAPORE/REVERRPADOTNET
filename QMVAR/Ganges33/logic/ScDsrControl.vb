...
Public Class ScDsrControl
    ...
    Public Function SelectScDsr(ByVal queryParams As ScDsrModel) As DataTable
        ...
        sqlStr = sqlStr & "AND rtrim(ltrim(ServiceOrder_No)) not in ('Total') "
    ...
    Public Function StoreManagement_drsCountsCancellation(ByVal queryParams As ScDsrModel) As DataTable
        Log4NetControl.ComInfoLogWrite(Log4NetControl.UserID)
            ...
            If _dtGetIsShipCodeChanged.Rows(0)("IsShipCodeChanged") = "0" Then
                ...
                If (IsShipChanged = True) Then
                    ...
                    If queryParams.DateFrom <= GetPreviousDate And queryParams.DateTo <= GetPreviousDate Then
                        ...
                        sqlStr = ""
                        sqlStr = sqlStr & " with cte as "
                        sqlStr = sqlStr & " ( "
                        sqlStr = sqlStr & " SELECT "
                        sqlStr = sqlStr & " COUNT(case when IW_total <> 0 AND OW_total = 0 then IW_total else NULL end ) AS CNT_IW_total, "
                        sqlStr = sqlStr & " COUNT(case when (OW_total <> 0 AND IW_total = 0) OR ( OW_total = 0 AND IW_total = 0 ) OR (IW_total <> 0 AND OW_total <> 0 ) then OW_total else NULL end ) AS CNT_OW_total "
                        sqlStr = sqlStr & " FROM dbo.SC_DSR WHERE DELFG = 0 AND Branch_name = @Branch_name "
                        sqlStr = sqlStr & " AND GoodsDelivered_date <> '0000/00/00' AND ServiceOrder_No <> 'Total'  "
                        sqlStr = sqlStr & " AND LEFT (CONVERT(VARCHAR, Billing_date, 111), 10)>= @DateFrom "
                        sqlStr = sqlStr & " AND LEFT(CONVERT(VARCHAR, Billing_date, 111), 10) <= @DateTo "
                        sqlStr = sqlStr & " AND serviceorder_no not in (select SERVICE_ORDER_NO from  SERVICE_ORDER_CANCELLED  where DELFG=0) "
                        sqlStr = sqlStr & " GROUP BY Billing_date "
                        sqlStr = sqlStr & " ) "
                        sqlStr = sqlStr & " select sum(CNT_IW_total) as iw_goods_total,sum(CNT_OW_total) as ow_goods_total,sum(CNT_IW_total)+sum(CNT_OW_total) as TotalGoods from cte "

                        Dim dbConn6 As DBUtility = New DBUtility()
                        If Not String.IsNullOrEmpty(branchname) Then
                            dbConn6.sqlCmd.Parameters.Add(CommonControl.GetNullableParameter("@Branch_name ", branchname))
                        End If
                        If (Not (String.IsNullOrEmpty(queryParams.DateFrom)) And (Not (String.IsNullOrEmpty(queryParams.DateTo)))) Then
    ...
    Public Function StoreManagement_DailyRevenue(ByVal queryParams As ScDsrModel) As DataTable
        ' queryParams.DateFrom = "2021/05/01"
        ...
        For Each branch As String In SplitBrances
            ...
            strSQL4 &= " left join "
            strSQL4 &= " ( "
            'Code commented VJ 2021/02/24 Saw Discount issue month first date 
            'strSQL4 &= " Select DSR.Billing_date, "
            ''--DSR.ServiceOrder_No
            ...
            'Code commented VJ 2021/02/24 Saw Discount issue month first date 
            ' VJ 2021/02/24 BEGIN Using SAW Discount calculation from PL Tracking Sheet Query
            'strSQL4 &= " select Billing_date,isnull(sum(Labor+Parts),0) SAWDISCOUNT from "
            'strSQL4 &= " ( "
            'strSQL4 &= " SELECT dbo.SC_DSR_info.Billing_date ,( "
            'strSQL4 &= " SELECT sum(Invoice_update.LABOR) "
            'strSQL4 &= " FROM  Invoice_update where upload_Branch = '" & branch & "' And Number = 'OWC' and Your_Ref_No in  "
            'strSQL4 &= " ( "
            'strSQL4 &= " select  distinct ASC_Claim_No "
            'strSQL4 &= " from Wty_Excel where Wty_Excel.DELFG!=1  AND  Wty_Excel.Branch_Code= (select ship_code from M_ship_base where ship_name='" & branch & "') "
            'strSQL4 &= " and  (  "
            'strSQL4 &= " (DAY(dbo.SC_DSR_info.Billing_date) = 1 and Wty_Excel.Delivery_Date between  LEFT(CONVERT(VARCHAR, DATEADD(D,-3,dbo.SC_DSR_info.Billing_date), 111), 10) and LEFT(CONVERT(VARCHAR, Billing_date, 111), 10) )   "
            'strSQL4 &= " or (DAY(dbo.SC_DSR_info.Billing_date) != 1 and Wty_Excel.Delivery_Date between  LEFT(CONVERT(VARCHAR, Billing_date, 111), 10) and LEFT(CONVERT(VARCHAR, Billing_date, 111), 10) )  "
            'strSQL4 &= " ))) as 'labor', "
            'strSQL4 &= " (SELECT sum(Invoice_update.Parts) FROM  Invoice_update where upload_Branch = '" & branch & "' AND Number = 'OWC' and Your_Ref_No in (  "
            'strSQL4 &= " select  distinct ASC_Claim_No from Wty_Excel where Wty_Excel.DELFG!=1 AND Wty_Excel.Branch_Code= (select ship_code from M_ship_base where ship_name='" & branch & "') "
            'strSQL4 &= " and  (  (DAY(dbo.SC_DSR_info.Billing_date) = 1 and Wty_Excel.Delivery_Date between  LEFT(CONVERT(VARCHAR, DATEADD(D,-3,dbo.SC_DSR_info.Billing_date), 111), 10)  "
            'strSQL4 &= " and LEFT(CONVERT(VARCHAR, Billing_date, 111), 10) ) or (DAY(dbo.SC_DSR_info.Billing_date) != 1  and Wty_Excel.Delivery_Date between  LEFT(CONVERT(VARCHAR, Billing_date, 111), 10)  "
            'strSQL4 &= " and LEFT(CONVERT(VARCHAR, Billing_date, 111), 10) )))) as 'parts'  "
            'strSQL4 &= " FROM dbo.SC_DSR_info WHERE DELFG = 0 And Branch_name = '" & branch & "' "
            'strSQL4 &= " AND dbo.SC_DSR_info.Billing_date between '" & queryParams.DateFrom & "' And '" & queryParams.DateTo & "' "
            'strSQL4 &= " ) SAWDCNT where Labor Is Not null Or Parts Is Not null  group by Billing_date "
            ' VJ 2021/02/24 END Using SAW Discount calculation from PL Tracking Sheet Query

            ' VJ 2021/02/26 BEGIN Using SAW Discount calculation from PL Tracking Sheet Query
            strSQL4 &= " SELECT DSR.ModBilling_date Billing_date,isnull(sum(INU.LABOR+INU.Parts),0) SAWDISCOUNT "
            'strSQL4 &= " --,sum(INU.LABOR) as Labor,sum(INU.Parts) as Parts  "
            strSQL4 &= " FROM  Invoice_update INU "
            strSQL4 &= " JOIN "
            strSQL4 &= " ( "
            strSQL4 &= " Select distinct ASC_Claim_No from Wty_Excel where Wty_Excel.DELFG!=1  And  Wty_Excel.Branch_Code='" & strShipCode & "'"
            strSQL4 &= " ) "
            strSQL4 &= " WE ON WE.ASC_Claim_No = INU.Your_Ref_No "
            strSQL4 &= " JOIN "
            strSQL4 &= " ( "
            strSQL4 &= " Select distinct ServiceOrder_No,SC_DSR.Billing_date,case when MONTH('" & queryParams.DateFrom & "') = month(SC_DSR.Billing_date) "
            strSQL4 &= " then SC_DSR.Billing_date else '" & queryParams.DateFrom & "' end ModBilling_date "
            strSQL4 &= " from SC_DSR where SC_DSR.DELFG!=1  "
            strSQL4 &= " and (SC_DSR.Billing_date between LEFT(CONVERT(VARCHAR, DATEADD(D,-3,'" & queryParams.DateFrom & "'), 111), 10) "
            strSQL4 &= " and LEFT(CONVERT(VARCHAR, '" & queryParams.DateTo & "', 111), 10)) "
            strSQL4 &= " ) "
            strSQL4 &= " DSR ON DSR.ServiceOrder_No = INU.Your_Ref_No "
            strSQL4 &= " where    upload_Branch in (" & shipName & ") AND Number = 'OWC' "
            strSQL4 &= " Group by DSR.ModBilling_date "
            ' VJ 2021/02/26 END Using SAW Discount calculation from PL Tracking Sheet Query
    ...
    Public Function StoreManagement_CallLoadRevenue(ByVal queryParams As ScDsrModel) As DataTable
        ...
        For Each branch As String In SplitBrances
            ...
            strSQL4 &= " isnull(TS.TARGET_CALLLOAD_COUNT,0) TARGET_DAY_AMOUNT, "

            'strSQL4 &= " (isnull(IWOW.IWOW_Total,0) + isnull(SAWD.SAWDISCOUNT,0)) DRS_AMOUNT, "
            strSQL4 &= " (isnull(IWOW.Goods_Total,0)) DRS_AMOUNT, "
            'strSQL4 &= " SUM (isnull(TS.TARGET_DAY_AMOUNT,0)) OVER (ORDER BY TDATE.Date) AS ACCUM_TARGET_DAY_AMOUNT, "

            strSQL4 &= " SUM (isnull(TS.TARGET_CALLLOAD_COUNT,0)) OVER (ORDER BY TDATE.Date) AS ACCUM_TARGET_DAY_AMOUNT, "

            'strSQL4 &= " SUM((isnull(IWOW.IWOW_Total, 0) + isnull(SAWD.SAWDISCOUNT, 0))) OVER (ORDER BY TDATE.Date) As ACCUM_DRS_AMOUNT "
            strSQL4 &= " SUM((isnull(IWOW.Goods_Total, 0) + isnull(0, 0))) OVER (ORDER BY TDATE.Date) As ACCUM_DRS_AMOUNT "
            ...
            strSQL4 &= " ( "
            'strSQL4 &= " Select DSR.Billing_date, "
            strSQL4 &= " Select DSR.Billing_date, isnull(sum(IW_goods_total+OW_goods_total),0.0) 'Goods_Total', "
            strSQL4 &= " isnull(sum(IW_Labor_total+IW_Transport_total+IW_Others_total),0.0) 'In_Warranty', "
            strSQL4 &= " isnull(sum(OW_Labor_total+OW_Parts_total+OW_Transport_total+OW_Others_total),0.00) 'Out_Warranty', "
            strSQL4 &= " isnull(sum(IW_Labor_total+IW_Transport_total+IW_Others_total+OW_Labor_total+OW_Parts_total+OW_Transport_total+OW_Others_total),0) IWOW_Total "
            strSQL4 &= " from dbo.SC_DSR_info DSR "
