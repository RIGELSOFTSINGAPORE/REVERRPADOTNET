...
Public Class AppleAppPartsInventoryControl

    Public Function SelectInventoryNo(queryParams As AppleAppPartsInventoryModel) As String
        ...
        If PoNo <> "-1" Then
            ...
            sqlStr = "Insert into AP_PARTS_INVENTORY ("
            sqlStr = sqlStr & "CRTDT, "
            sqlStr = sqlStr & "CRTCD, "
            sqlStr = sqlStr & "UPDDT, "
            sqlStr = sqlStr & "UPDCD, "
    ...
    Public Function SelectInventoryEdit(queryParams As AppleAppPartsInventoryModel) As DataTable
        Log4NetControl.ComInfoLogWrite(Log4NetControl.UserID)
        Dim DateTimeNow As DateTime = DateTime.Now
        Dim dtNow As DateTime = DateTimeNow.AddMinutes(ConfigurationManager.AppSettings("TimeDiff"))
    ...
    Public Function GetInventry(queryParams As AppleAppPartsInventoryModel) As DataTable
        Dim DateTimeNow As DateTime = DateTime.Now
        Dim dtNow As DateTime = DateTimeNow.AddMinutes(ConfigurationManager.AppSettings("TimeDiff"))
        Dim flag As Boolean = False
        ...
        sqlStr = sqlStr & "INVENTORY_NO,FORMAT(INVENTORY_DATE, 'yyyy/MM/dd') as INVENTORY_DATE,FORMAT(UPDDT, 'yyyy/MM/dd') as UPDDT,PARTS_TYPE "
        sqlStr = sqlStr & "FROM "
        sqlStr = sqlStr & "AP_PARTS_INVENTORY "
        sqlStr = sqlStr & "WHERE "
        sqlStr = sqlStr & "DELFG=0 "
    ...
    Public Function GetpartsEntry(queryParams As AppleAppPartsInventoryModel) As DataTable
        Dim DateTimeNow As DateTime = DateTime.Now
        Dim dtNow As DateTime = DateTimeNow.AddMinutes(ConfigurationManager.AppSettings("TimeDiff"))
        Dim flag As Boolean = False
        ...
        If Not String.IsNullOrEmpty(queryParams.PARTS_TYPE) Then
            ...
            If (queryParams.PARTS_TYPE = "Consignment") Then
                sqlStr = "SELECT "
                sqlStr = sqlStr & "ROW_NUMBER() OVER (ORDER BY INVENTORY_NO) RowNumber,PARTS_NO,PARTS_NAME,PARTS_DESCRIPTION,PURCHASE_COST,IN_STOCK,SALES_COST,FORMAT(INVENTORY_DATE, 'yyyy/MM/dd') as INVENTORY_DATE,PARTS_TYPE "
                sqlStr = sqlStr & "FROM "
        ...
        End If
        'If Not String.IsNullOrEmpty(queryParams.PARTS_TYPE) Then
        '    sqlStr = sqlStr & "AND PO_NO = @PARTS_TYPE "
        '    dbConn.sqlCmd.Parameters.Add(CommonControl.GetNullableParameter("@PO_NO", queryParams.PARTS_TYPE))
        'End If
