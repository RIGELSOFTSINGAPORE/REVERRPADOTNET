...
Public Class Class_analysis
    ...
    Public Sub setCsvData(ByVal csvData()() As String, ByVal userid As String, ByVal userName As String, ByVal uploadShipname As String, ByVal setShipCode As String, ByRef errFlg As Integer, ByVal dailyKind As String, ByRef billingDateAll() As String, ByVal FileName As String)
            ...
            For i = 0 To csvData.Length - 1
                ...
                If i <> 0 AndAlso i <> 1 Then
                    ...
                    If newData = 1 Then
                        ...
                        'Log4NetControl.ComInfoLogWrite("New: " & csvData(i)(0))
                    End If
        ...
        Catch ex As Exception
            trn.Rollback()
            'Log4NetControl.ComInfoLogWrite(ex.Message)
            errFlg = 1
        Finally
            'DB接続クローズ
        ...
        Try
            ...
            'Revert Back 20210721
            'CR  - If the total of both IW and OOW is 0, it is considered that the repair was created but canceled.
            'CR - Therefore, if the total amount is 0 for both IW and OOW, it will be excluded from the count.
            'select_sql4 &= "SELECT Billing_date, "
            'select_sql4 &= "COUNT(case when IW_Labor <> 0 then IW_Labor else NULL end ) AS CNT_IW_Labor, "
            'select_sql4 &= "COUNT(case when OW_Labor <> 0 then OW_Labor else NULL end ) AS CNT_OW_Labor, "
            'select_sql4 &= "COUNT(case when IW_total <> 0 AND OW_total = 0 then IW_total else NULL end ) AS CNT_IW_total, "
            ''OR ( OW_total = 0 AND IW_total = 0 ) Removed
            'select_sql4 &= "COUNT(case when (OW_total <> 0 AND IW_total = 0)  OR (IW_total <> 0 AND OW_total <> 0 ) then OW_total else NULL end ) AS CNT_OW_total "
    ...
    Public Sub setCsvDataWtyExcelDown(ByVal csvData()() As String, ByVal userid As String, ByVal userName As String, ByRef errFlg As Integer, ByRef tourokuFlg As Integer, ByVal uploadShipname As String, ByVal setShipCode As String, ByVal FileName As String)
        ...
        Dim strCommaReplace As String = ""
        Dim strYear As String = ""
        Dim isDeliveryDateValidFmt As Boolean = True
            ...
            For i = 0 To csvData.Length - 1
                ...
                If i <> 0 Then
                    ...
                    If (csvData(i)(1) = setShipCode) Or (csvData(i)(1) = ParentShipCode And ParentShipCode <> "") Then
                        newData = -1
                        add = -1
                        'Log4NetControl.ComErrorLogWrite(i & "  " & csvData(i)(3))
                        isDeliveryDateValidFmt = True
                        Dim select_sql1 As String = ""
                        ...
                        If csvData(i)(24) = "00000000" Then
                            'select_sql1 &= "AND Delivery_Date IS NULL"
                            isDeliveryDateValidFmt = False
                        Else
                        ...
                        strCommaRepl = Trim(csvData(i)(6))
                        strCommaReplace = Replace(strCommaRepl, ",", " ")
                        dr1("Consumer_Addr1") = strCommaReplace

                        strCommaRepl = Trim(csvData(i)(7))
                        strCommaReplace = Replace(strCommaRepl, ",", " ")
                        dr1("Consumer_Addr2") = strCommaReplace
                        ...
                        Else
                            ...
                            Else
                                ...
                                Dim dateV As DateTime = dr1("Purchase_Date")
                                strYear = dateV.Year
                                If strYear <= 1953 Then
                                    dr1("Purchase_Date") = DBNull.Value
                                End If
                            End If
                        End If
                        ...
                        '新規DRをDataTableに追加
                        If newData = 1 Then
                            If isDeliveryDateValidFmt = True Then
                                ds1.Tables(0).Rows.Add(dr1)
                            End If
