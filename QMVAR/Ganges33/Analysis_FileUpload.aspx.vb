...
Public Class Analysis_FileUpload
    Inherits System.Web.UI.Page
    '8 Good Receiving 1291
    ...
    Protected Sub btnUpload_Click(sender As Object, e As EventArgs) Handles btnUpload.Click
        ...
        If DropListLocation.Text = "Select Branch" Then
            Call showMsg("Please select the Branch.", "")
            Exit Sub
        End If

        If drpTask.SelectedValue = "0" Then
            Call showMsg("Please select the file name", "")
            Exit Sub
        End If
        '**************************************************************
        '
        ...
        Else
            ...
            '24 - no need to file upload
            If Not drpTask.SelectedValue = "24" Then
                If Not ((strExtension = ".csv") Or (strExtension = ".pdf")) Then 'strExtension = ".csv"
            ...
            ElseIf drpTask.SelectedValue = "24" Then
                ...
                End If
            ElseIf drpTask.SelectedValue = "27" Then

                'Item27 SERVICE_ORDER_PENDING
                '*********************************
                'text box validation
                If String.IsNullOrEmpty(txtstatus.Text) Then
                    Call showMsg("Please Enter Status ", "")
                    Exit Sub
                End If

                If String.IsNullOrEmpty(txtReason.Text) Then
                    Call showMsg("Please Enter Reason ", "")
                    Exit Sub
                End If

                If String.IsNullOrEmpty(txtAgingDay.Text) Then
                    Call showMsg("Please Enter Aging Day ", "")
                    Exit Sub
                End If
                ...
                FileUploadAnalysis.SaveAs(filenameFullPath)
                'Update to Database
                Item27(filenameFullPath, fileName, userid, userName, strSrcFileName)

            ElseIf drpTask.SelectedValue = "26" Then
                '26.Service Order Cancelled
                '*********************************
                'Verify FileName Rules Followed
                Dim strFileName As String = System.IO.Path.GetFileNameWithoutExtension(FileUploadAnalysis.FileName)
                Dim strSrcFileName As String = System.IO.Path.GetFileName(FileUploadAnalysis.FileName)
                ...
                FileUploadAnalysis.SaveAs(filenameFullPath)
                'Update to Database
                Item26(filenameFullPath, fileName, userid, userName, strSrcFileName)


            ElseIf (drpTask.SelectedValue = "3a1") Or (drpTask.SelectedValue = "3b1") Or (drpTask.SelectedValue = "4a") Then
                '3A-1.PDF-Sales Register from GSPN Samsung_C IW
                '3B-1,PDF-Sales Register from GSPN Samsung_EXC IW
                '4A,PDF-Sales Register from GSPN OOW
                '*********************************
                Dim strExtension1 As String = System.IO.Path.GetExtension(FileUploadAnalysis.FileName)

                If Not (strExtension1 = ".pdf") Then
                    Call showMsg("Only Allowed to upload PDF file!!!", "")
                    Exit Sub
                End If

                Dim strFileName As String = System.IO.Path.GetFileNameWithoutExtension(FileUploadAnalysis.FileName)
                Dim FullFileName As String = System.IO.Path.GetFileName(FileUploadAnalysis.FileName)


                Dim _InvoiceUpdatePdfModel As InvoiceUpdatePdfModel = New InvoiceUpdatePdfModel()
                strFileName = FileUploadAnalysis.FileName

                'Sample Format
                'SSC1_202108_C550INR23253070188_1632740875006
                Dim strFileSplit() As String
                strFileSplit = Split(strFileName, "_")

                Dim strSSC As String = ""
                strSSC = Trim(strFileSplit(0))
                'Verify SSC match with dropdown selected  name and file name 
                If Not (strSSC = DropListLocation.SelectedItem.Text) Then
                    Call showMsg("File Name is not match with Selected SSC", "")
                    Exit Sub
                End If

                If strFileSplit.Count <= 2 Then
                    Call showMsg("File Name is not Valid Format!!!", "")
                    Exit Sub
                End If


                Dim strMonthFolder As String = ""
                strMonthFolder = strFileSplit(1)

                _InvoiceUpdatePdfModel.FiscalMonth = strMonthFolder

                _InvoiceUpdatePdfModel.ReportNo = strFileSplit(2)

                'strFileSplit(2) & (3)  - For File Name of original

                Select Case drpTask.SelectedItem.Value
                    Case "3a1"

                        If Len(strFileName) > 2 Then
                            If Not (UCase(Left(strFileSplit(2), 1)) = "C") Then
                                Call showMsg("The pdf file name is Invalid", "")
                                Exit Sub
                            End If
                        Else
                            Call showMsg("The pdf file name is Invalid", "")
                    ...
                    Case "3b1"
                        ...
                        If Len(strFileName) > 4 Then
                            If Not (UCase(Left(strFileSplit(2), 3)) = "EXC") Then
                                Call showMsg("The pdf file name is Invalid", "")
                                Exit Sub
                            End If
                        Else
                    ...
                    Case "4a"
                        ...
                        If Len(strFileName) > 4 Then
                            If Not (UCase(Left(strFileSplit(2), 3)) = "OWC") Then
                                Call showMsg("The pdf file name is Invalid", "")
                                Exit Sub
                            End If
                        Else
                ...
                If TextPartsInvoiceNo.Text = "" Then
                    ...
                    Exit Sub
                Else
                    _InvoiceUpdatePdfModel.PartsInvoiceNo = Trim(TextPartsInvoiceNo.Text)
                End If
                ...
                If TextLaborInvoiceNo.Text = "" Then
                    ...
                    Exit Sub
                Else
                    _InvoiceUpdatePdfModel.LaborInvoiceNo = Trim(TextLaborInvoiceNo.Text)
                End If

                If TextInvoiceDate.Text <> "" Then
                    If DateTime.TryParse(TextInvoiceDate.Text, dt) Then
                        _InvoiceUpdatePdfModel.InvoiceDate = DateTime.Parse(Trim(TextInvoiceDate.Text))
                    Else
                        Call showMsg("The date specification of InvoiceDate is incorrect.", "")
                        Exit Sub
                    End If
                ...
                If Not Directory.Exists(strRooPath) Then
                    Directory.CreateDirectory(strRooPath)
                End If
                strPath = strRooPath & "\pdf"
                If Not Directory.Exists(strPath) Then
                    Directory.CreateDirectory(strPath)
                End If

                strPath = strRooPath & "\pdf" & "\" & strMonthFolder
                If Not Directory.Exists(strPath) Then
                    Directory.CreateDirectory(strPath)
                End If

                Dim dirPath As String = strPath
                Dim fileName As String = strFileName & "_" & DateTime.Now.ToString("yyyyMMddHHmmssfff") & strExtension1
                Dim filenameFullPath As String = dirPath & "\" & fileName
                'Save the file in temporary folder
                FileUploadAnalysis.SaveAs(filenameFullPath)

                strPath = "pdf" & "\" & strMonthFolder & "\" & fileName

                'Pass the paramters to Update in Datatabase
                _InvoiceUpdatePdfModel.FileName = strPath
                _InvoiceUpdatePdfModel.SrcFileName = strFileSplit(2) & "_" & strFileSplit(3)

                _InvoiceUpdatePdfModel.UPDPG = "Class.Analysis.vb"
                _InvoiceUpdatePdfModel.UserId = userid
                _InvoiceUpdatePdfModel.UploadUser = userName
                ...
                _InvoiceUpdatePdfModel.FileNameSSC = strSSC
                ...
                Dim blStatus As Boolean = _InvoiceUpdatePdfControl.AddModifyInvoiceUpdatePdf(_InvoiceUpdatePdfModel)
                'If Error Raised then show the errors
    ...
    Sub DefaultSearchSetting(ByVal Optional SearchType As String = "init")
        If SearchType = "init" Then
            ...
            txtAgingDay.Visible = False
            lblstatus.Visible = False
            lblReason.Visible = False
            lblAgingday.Visible = False

        ElseIf SearchType = "SERVICE_ORDER_PENDING" Then
            TextPartsInvoiceNo.Enabled = False
            TextLaborInvoiceNo.Enabled = False
            TextInvoiceDate.Enabled = False
            txtDate.Enabled = False
            ...
            txtAgingDay.Visible = True
            lblstatus.Visible = True
            lblReason.Visible = True
            lblAgingday.Visible = True

        ElseIf SearchType = "SERVICE_ORDER_Cancell" Then
            txtstatus.Visible = False
            txtReason.Visible = False
            txtAgingDay.Visible = False
            lblstatus.Visible = False
    ...
    Protected Sub OnSelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        ElseIf drpTask.SelectedValue = "26" Then
            DefaultSearchSetting("SERVICE_ORDER_Cancell")
        ElseIf drpTask.SelectedValue = "27" Then
            DefaultSearchSetting("SERVICE_ORDER_PENDING")
        End If
    End Sub
    ...
    Private Sub Item27(filenameFullPath As String, fileName As String, userid As String, userName As String, strSrcFileName As String)
        'Read the content from stored file
        Dim cReader
        Dim textLines As New List(Of String())
        Dim strArr()() As String
        ...
        _ServiceOrderPendingModel.UPDPG = "Class.Analysis.vb"
        _ServiceOrderPendingModel.FileName = fileName
        _ServiceOrderPendingModel.SrcFileName = strSrcFileName
        _ServiceOrderPendingModel.UserId = userid
        _ServiceOrderPendingModel.UploadUser = userName
        ...
        _ServiceOrderPendingModel.Status = txtstatus.Text
        _ServiceOrderPendingModel.Reason = txtReason.Text
        _ServiceOrderPendingModel.AGING_DAY = txtAgingDay.Text

        'Upload to csv file to database for Other Update (GSPN to MultiVendor)
        Dim blStatus As Boolean = _ServiceOrderPendingControl.AddModifyServiceOrderPendingControl(strArr, _ServiceOrderPendingModel)
        'If Error Raised then show the errors
        If Not blStatus Then
            Call showMsg("Status: Upload Failed   Please check the content of file ", "")
            Exit Sub
    ...
    Private Sub Item26(filenameFullPath As String, fileName As String, userid As String, userName As String, strSrcFileName As String)
        'Read the content from stored file
        Dim cReader
        Dim textLines As New List(Of String())
        Dim strArr()() As String
        ...
        _ServiceOrderCancelledModel.UPDPG = "Class.Analysis.vb"
        _ServiceOrderCancelledModel.FileName = fileName
        _ServiceOrderCancelledModel.SrcFileName = strSrcFileName
        _ServiceOrderCancelledModel.UserId = userid
        _ServiceOrderCancelledModel.UploadUser = userName
        ...
        'Upload to csv file to database for Other Update (GSPN to MultiVendor)
        Dim blStatus As Boolean = _ServiceOrderCancelledControl.AddModifyServiceOrderCancelledControl(strArr, _ServiceOrderCancelledModel)
        'If Error Raised then show the errors
        If Not blStatus Then
            Call showMsg("Status: Upload Failed   Please check the content of file ", "")
            Exit Sub
    ...
    Private Sub Item11(filenameFullPath As String, fileName As String, userid As String, userName As String, strSrcFileName As String)
        ...
        Dim blStatus As Boolean = _PartsIoControl.AddModifyPartsIoNew(strArr, _PartsIoModel)
        'If Error Raised then show the errors
        If Not blStatus Then
            Call showMsg("Status: Upload Failed     Please check the content of file ", "")
            Exit Sub
    ...
    Private Sub Item3a1pdf(filenameFullPath As String, fileName As String, userid As String, userName As String, strSrcFileName As String)
        'Read the content from stored file
        Dim cReader
        Dim textLines As New List(Of String())
        Dim strArr()() As String
