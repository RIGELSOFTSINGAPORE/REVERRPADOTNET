...
#Region " Windows フォーム デザイナで生成されたコード "
    ...
    Friend WithEvents corp_flg As System.Windows.Forms.ComboBox
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.corp_flg = New System.Windows.Forms.ComboBox()
        ...
        'corp_flg
        '
        Me.corp_flg.DropDownStyle = System.Windows.Forms.ComboBoxStyle.DropDownList
        Me.corp_flg.Font = New System.Drawing.Font("Arial", 11.25!, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, CType(0, Byte))
        Me.corp_flg.ImeMode = System.Windows.Forms.ImeMode.Off
        Me.corp_flg.Items.AddRange(New Object() {"0-個人", "1-法人"})
        Me.corp_flg.Location = New System.Drawing.Point(128, 61)
        Me.corp_flg.Name = "corp_flg"
        Me.corp_flg.Size = New System.Drawing.Size(80, 25)
        Me.corp_flg.TabIndex = 10
        '
        'Label23
        '
...
#End Region
    ...
    Private Sub Form2_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        SqlSelectCommand2 = New SqlClient.SqlCommand("SELECT cd1, cd2, cd3, [品種名_漢字] as cat_name, [品種名_ｶﾅ] as cat_kana, avlbty, GRP FROM cat_mtr , cnsqlclient)
'WHERE (BY_cls = 'Y')", cnsqlclient)
        ...
        SqlSelectCommand4 = New SqlClient.SqlCommand("SELECT shop_code, [店舗名_漢字] as shop_name, close_date FROM shop_mtr , cnsqlclient)
'WHERE (BY_cls = 'Y')", cnsqlclient)
    ...
    Sub disp_main()
        ...
        If dttbl_mtr.Rows(0)("corp_flg") = "0" Then
            corp_flg.SelectedItem = "0-個人(5.0%)"
        ElseIf dttbl_mtr.Rows(0)("corp_flg") = "1" Then
            corp_flg.SelectedItem = "1-法人"
        ElseIf dttbl_mtr.Rows(0)("corp_flg") = "2" Then
            corp_flg.SelectedItem = "2-個人(5.238%)"
        End If
    ...
    Sub disp_sub()
        ...
        For Line_No = 0 To dttbl_sub.Rows.Count - 1
            ...
            SqlSelectCommand3 = New SqlClient.SqlCommand("SELECT vdr_code, RTRIM(vdr_name) AS Name FROM vdr_mtr WHERE (BY_cls = 'Y') ORDER BY use_date DESC, vdr_name", cnsqlclient)
            ...
            edit(Line_No, x).TextVAlign = GrapeCity.Win.Input.Interop.AlignVertical.Middle
            edit(Line_No, x).Text = Mid(dttbl_sub.Rows(Line_No)("item_cat_code"), 1, 2)
            Panel1.Controls.Add(edit(Line_No, x))
            ...
            edit(Line_No, x).TextHAlign = GrapeCity.Win.Input.Interop.AlignHorizontal.Center
            edit(Line_No, x).TextVAlign = GrapeCity.Win.Input.Interop.AlignVertical.Middle
            edit(Line_No, x).Text = Mid(dttbl_sub.Rows(Line_No)("item_cat_code"), 3, 2)
            Panel1.Controls.Add(edit(Line_No, x))
            ...
            DtView1 = New DataView(dsItem2.Tables("cat"), "cd1 ='" & edit(Line_No, 1).Text & "' AND cd2 ='" & edit(Line_No, 2).Text & "' AND cd3 = '00'", "", DataViewRowState.CurrentRows)
            label(Line_No, x).Text = DtView1(0)("cat_name")
            ...
            label(Line_No, x).Text = Format(dttbl_sub.Rows(Line_No)("wrn_fee"), "##,##0")
            Panel1.Controls.Add(label(Line_No, x))
    ...
    Sub ADD_LINE()
        ...
        'メーカー
        Dim SqlSelectCommand3 As New SqlClient.SqlCommand
        SqlSelectCommand3 = New SqlClient.SqlCommand("SELECT vdr_code, vdr_name FROM vdr_mtr WHERE (BY_cls = 'Y') ORDER BY use_date DESC, vdr_name", cnsqlclient)
        SqlSelectCommand3.CommandType = CommandType.Text
        Dim daItem3 As New SqlClient.SqlDataAdapter
        daItem3.SelectCommand = SqlSelectCommand3
    ...
    Sub modify()
        ...
        If r < 2 Then
            ...
            strSQL += ", corp_flg = '" & Mid(corp_flg.Text, 1, 1) & "'"
            ...
            For i = 0 To Line_No
                If label(i, 0).Text <> Nothing Then
                    If label(i, 2).Text <> Nothing Then
                        ...
                        End If
                        strSQL += ", prch_price = " & (num(i, 0).Value / 1.05) & ""
                        strSQL += ", prch_tax = " & (num(i, 0).Value - num(i, 0).Value / 1.05) & ""
                        strSQL += ", prch_date = CONVERT(DATETIME, '" & datBox(i, 0).Text & "', 102)"
                        strSQL += ", item_cat_code = '" & edit(i, 1).Text & edit(i, 2).Text & "'"
                        DtView1 = New DataView(dsItem2.Tables("cat"), "cd1 ='" & edit(i, 1).Text & "' AND cd2 ='" & edit(i, 2).Text & "' AND cd3 = '00'", "", DataViewRowState.CurrentRows)
                        strSQL += ", cat_name = '" & DtView1(0)("cat_kana") & "'"
                        strSQL += ", bend_code = '" & cmbbox(i, 0).SelectedValue & "'"
                        'strSQL +=  ", brnd_name = '" & cmbbox(i, 0).SelectedText & "'"
                        strSQL += ", model_name = '" & edit(i, 0).Text & "'"
                        strSQL += ", pos_code = '" & edit(i, 3).Text & "'"
                        strSQL += ", wrn_fee = " & CInt(label(i, 1).Text) & ""
                        strSQL += ", op_date = CONVERT(DATETIME, " & Date_ent.Text & ", 102)"
                        strSQL += " WHERE ordr_no = '" & dttbl_mtr.Rows(0)("ordr_no") & "' AND line_no = '" & label(i, 2).Text & "'"
                        Try
                            DB_OPEN("best_wrn")
                            sqlUpdateCommand = New SqlClient.SqlCommand(strSQL, cnsqlclient)
                            sqlUpdateCommand.ExecuteNonQuery()
    ...
    Private Sub prch_Leave(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        If Mid(corp_flg.SelectedItem, 1, 1) <> "0" And Mid(corp_flg.SelectedItem, 1, 1) <> "1" And Mid(corp_flg.SelectedItem, 1, 1) <> "2" Then
            MessageBox.Show("契約タイプを指定してください。", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error)
            corp_flg.Focus()
            Exit Sub
        Else
            If num(numbox.Tag, 0).Value >= 10500 Then
                If Mid(corp_flg.SelectedItem, 1, 1) = "0" Then
                    ...
                    lbl_wrnttl.Text = 0
                ElseIf Mid(corp_flg.SelectedItem, 1, 1) = "1" Then
    ...
    Private Sub corp_flg_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles corp_flg.SelectedIndexChanged
        If k <> 0 Then
            If r = 1 Then
                For i = 0 To Line_No
                    If Mid(corp_flg.SelectedItem, 1, 1) = "0" Then
