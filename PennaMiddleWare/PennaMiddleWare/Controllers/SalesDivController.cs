...
{
    public class SalesDivController : Controller
    {
        ...
        public ActionResult Index(string startDate, string endDate, string InputID, int? page, int? page1)
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("Select * from ZSD_MW_SALES_DIS_SRV where  ZSD_MW_SALES_DIS_SRV_PKID = " + Maxid))
                {
                    ...
                    using (MySqlDataReader sdr = cmd.ExecuteReader())
                    {
                        while (sdr.Read())
                        {
                            lstsrvdeal.Add(new ZSD_MW_SALES_DIS_SRV
                            {
                                ...
                                ProRata = Convert.ToString(sdr["ProRata"]),
                                PrRrata = Convert.ToString(sdr["PrRrata"]),
            ...
            if (InputID == "E")
            {
                DataTable dt1 = new DataTable("ZSD_MW_SALES_DIS_SRV");
                dt1.Columns.AddRange(new DataColumn[19]
                {
                        ...
                        new DataColumn("Total Prorata Target"),
                        new DataColumn("Prorata Target per D"),
                        new DataColumn("Prorata Target per D1"),
                        new DataColumn("Plant Transfer OnDat"),
                        new DataColumn("Plant Transfer Month")
                 });

                using (MySqlConnection con = new MySqlConnection(CS))
                {
                    string query = " Select * from ZSD_MW_SALES_DIS_SRV ";
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query))
                    {
                        ...
                        using (MySqlDataReader sdr = cmd.ExecuteReader())
                        {
                            while (sdr.Read())
                            {
                                ...
                                _dr["Total Prorata Target"] = Convert.ToString(sdr["ProRata"]);
                                _dr["Prorata Target per D"] = Convert.ToString(sdr["PrRrata"]);
                                _dr["Prorata Target per D1"] = Convert.ToString(sdr["PrRerata"]);
                                _dr["Plant Transfer OnDat"] = Convert.ToString(sdr["PltrOndate"]);
                                _dr["Plant Transfer Month"] = Convert.ToString(sdr["PltrMondate"]);
                ...
                using (XLWorkbook wb = new XLWorkbook())
                {
                    ...
                    {
                        wb.SaveAs(stream);
                        return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "ZSD_MW_SALES_DIS_SRV.xlsx");
                    }
                }
            }
            ...
            if (InputID == "S")
            {
                lstsrvdeal = GetSalesDivDetails(startDate, endDate);
        ...
        public ActionResult Index(string startDate, string endDate, string InputID, int? page)
        {
            ...
            try
            {
                ...
                if (InputID == "I")
                {
                    ...
                    //Prod
                    //var url = "https://netwaver-prd.pennacement.com:443/sap/opu/odata/sap/ZSD_MW_SALES_DIS_SRV/SALES_DISSet/?$filter=(BillDate ge '" + startDateOrig.Trim() + "' and BillDate le '" + endDateOrig.Trim() + "')&$format=json";
                    //string userName = "NWGW037";
                    //string passwd = "Admin@123456";


                    //Dev
                    var url = "https://netwaver-dev.pennacement.com:443/sap/opu/odata/sap/ZSD_MW_SALES_DIS_SRV/SALES_DISSet/?$filter=(BillDate ge '" + startDateOrig.Trim() + "' and BillDate le '" + endDateOrig.Trim() + "')&$format=json";
                    string userName = "NWGW001";
                    string passwd = "Penna@123";

                    log1.WriteLine("Insert Table : ZSD_MW_SALES_DIS_SRV");
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        query = "";
                        query = " Select Count(*) from ZSD_MW_SALES_DIS_SRV ";
                        query += " where date_format(cast( str_to_date(Billdate,'%d.%m.%Y') as date),'%Y%m%d')  ";
                        query += " between '" + startDate.Trim() + "' and '" + endDate.Trim() + "'";
            ...
            catch (Exception ex)
            {
                ...
                log1.WriteLine("ZSD_MW_SALES_DIS_SRV Error Message : " + ex.Message);
                log1.WriteLine("===========================================");
                log1.Close();
        ...
        private List<ZSD_MW_SALES_DIS_SRV> GetSalesDivDetails(string startDate, string endDate)
        {
            List<ZSD_MW_SALES_DIS_SRV> lstsrvdeal = new List<ZSD_MW_SALES_DIS_SRV>();
