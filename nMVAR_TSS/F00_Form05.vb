...
#End Region
    ...
    Sub inz()
        ...
        strSQL = strSQL & ", '' AS tel, '' AS makr_code, '' AS maker_name, '' AS modl_name"
        strSQL = strSQL & ", '' AS s_no, 0 AS prch_amnt, '' AS wrn_tem, '' AS wrn_tem_name"
        strSQL = strSQL & ", '' AS makr_wrn_stat, '' AS wrn_range, '' AS wrn_range_name, '' AS univ_name"
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        strSQL = strSQL & ", T01_member.makr_code, M05_VNDR.name AS maker_name, T01_member.modl_name, T01_member.s_no"
        strSQL = strSQL & ", T01_member.prch_amnt, T01_member.wrn_tem, V_M02_HSK.cls_code_name AS wrn_tem_name"
        strSQL = strSQL & ", T01_member.makr_wrn_stat, T01_member.wrn_range, V_M02_HSY.cls_code_name AS wrn_range_name"
        strSQL = strSQL & ", M01_univ.univ_name"
        strSQL = strSQL & " FROM T01_member INNER JOIN"
        strSQL = strSQL & " M01_univ ON T01_member.univ_code = M01_univ.univ_code LEFT OUTER JOIN"
        strSQL = strSQL & " M05_VNDR ON T01_member.makr_code = M05_VNDR.vndr_code LEFT OUTER JOIN"
        strSQL = strSQL & " V_M02_HSY ON T01_member.wrn_range = V_M02_HSY.cls_code LEFT OUTER JOIN"
        strSQL = strSQL & " V_M02_HSK ON T01_member.wrn_tem = V_M02_HSK.cls_code"
        strSQL = strSQL & " WHERE (T01_member.delt_flg = 0)"
        If Edit001.Text <> Nothing Then
            strSQL = strSQL & " AND (T01_member.user_name LIKE '%" & Label1.Text & "%'"
            strSQL = strSQL & " OR T01_member.user_name LIKE '%" & Label2.Text & "%'"
            strSQL = strSQL & " OR T01_member.user_name LIKE '%" & Label3.Text & "%'"
            strSQL = strSQL & " OR T01_member.user_name LIKE '%" & Label4.Text & "%'"
            strSQL = strSQL & " OR T01_member.user_name LIKE '%" & Label5.Text & "%')"
        End If
        If Edit003.Text <> Nothing Then
            strSQL = strSQL & " AND (T01_member.tel LIKE '" & Edit003.Text & "%')"
        End If
        SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
        DaList1.SelectCommand = SqlCmd1
        DB_OPEN("QGCare")
