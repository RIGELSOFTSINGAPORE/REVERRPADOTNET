Module Module_Sub
                                    ...
                                    If DtView1(0)("grup_code") = "90" Then  '②
                                        If wrn_limt_amnt < sTTL Then        '限度オーバー
                                            If menseki_amnt <> 0 Then           '動産扱い
                                                ...
                                                Else
                                                    P_idvd = sTTL - wrn_limt_amnt + 4762                '自己負担
                                                End If
                                                If sTTL < 35000 Then
    ...
    Public Function QG_HSTY(ByVal qg_care_no As String, ByVal rcpt_no As String, ByVal acdt_date As Date, ByVal amnt As Integer) As Decimal
        ...
        strSQL = strSQL & " AND (rcpt_no = '" & rcpt_no & "')"
        ...
        If DtView1.Count = 0 Then
            ...
            strSQL = strSQL & " VALUES ('" & qg_care_no & "'"
            strSQL = strSQL & ", '" & rcpt_no & "'"
        ...
        Else
            ...
            strSQL = strSQL & " SET repr_chrg = " & amnt
            strSQL = strSQL & " WHERE (qg_care_no = '" & qg_care_no & "')"
            strSQL = strSQL & " AND (rcpt_no = '" & rcpt_no & "')"
            strSQL = strSQL & " AND (from_date <= CONVERT(DATETIME, '" & acdt_date & "', 102))"
            strSQL = strSQL & " AND (to_date >= CONVERT(DATETIME, '" & acdt_date & "', 102))"
    ...
    Public Function Fuji_neva(ByVal rcpt_no As String) As String
        ...
        strSQL = strSQL & ", T01_REP_MTR.qg_care_no, T01_REP_MTR.comp_shop_ttl, M08_STORE.grup_code, T01_REP_MTR.rcpt_cls"
        strSQL = strSQL & " FROM T01_REP_MTR INNER JOIN"
        strSQL = strSQL & " M08_STORE ON T01_REP_MTR.store_code = M08_STORE.store_code"
        strSQL = strSQL & " WHERE (T01_REP_MTR.rcpt_no = '" & rcpt_no & "')"
        SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
        DaList1.SelectCommand = SqlCmd1
        DB_OPEN("nMVAR")
        DaList1.Fill(DsList1, "T01_REP_MTR")
        ...
        Else
            ...
            Else
                ...
                strSQL = strSQL & " WHERE (rcpt_no = '" & rcpt_no & "') AND (kbn = '2')"
                SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
                DaList1.SelectCommand = SqlCmd1
                DB_OPEN("nMVAR")
                ...
                'strSQL = strSQL & " M01_insurance_co_decision ON T01_wrn_date.Securities_no = M01_insurance_co_decision.Securities_no"
                'strSQL = strSQL & " WHERE (T01_wrn_date.wrn_id = '" & DtView1(0)("qg_care_no") & "')"
                'SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
                'DaList1.SelectCommand = SqlCmd1
                'DB_OPEN("Fujitsu")
                ...
                Else                                    '有償
                    ...
                    Else
                        If part_cnt_exp <> 0 _
                            Or RoundDOWN(DtView1(0)("wrn_limt_amnt") / 1.08, 0) < DtView1(0)("comp_shop_ttl") Then    '消耗品あり or 限度額超過
                            Return "1"                  'E
                        Else
                            Return "0"                  'C D
                        End If
