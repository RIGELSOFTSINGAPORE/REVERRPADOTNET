Module Module_Sub
    ...
    Public Function PRT_GET(ByVal cls) As String
        ...
        If DtView1.Count <> 0 Then
            ...
            P_lowr_mrgn = DtView1(0)("lowr_mrgn")
    ...
    '**  プリンタ名取得(テスト印刷用)
    '********************************************************************
    Public Function PRT_test_GET(ByVal prt, ByVal moji) As String
        Dim WK_str As String
        P_mojibake = Nothing
    ...
    '**  保険限度額取得
    '********************************************************************
    Public Function Limit_Get(ByVal qg_care_no As String, ByVal acdt_stts As String, ByVal acdt_date As Date) As Integer
        ...
        If DtView1.Count <> 0 Then
            ...
            Select Case DtView1(0)("保証範囲")
                ...
                Case Is = "5"   'スタンダードプラス
                    Select Case acdt_stts
                        Case Is = "1", "3", "4" '故障、破損、水濡れ
                            wrn_F = "1"
                    End Select
                Case Is = "3"   'スペシャル
                    Select Case acdt_stts
                        Case Is = "1", "2", "3", "4" '故障、落雷、破損、水濡れ
                            wrn_F = "1"
                    End Select
                Case Is = "4"   'セーフティー
                    Select Case acdt_stts
                        Case Is = "2", "3", "4" '落雷、破損、水濡れ
                            wrn_F = "1"
                    End Select
                Case Is = "7"   '延長保証
                    Select Case acdt_stts
            ...
            If wrn_F = "1" Then '保証あり
                ...
                For i = 1 To 10
                    If DateAdd(DateInterval.Year, i, CDate(DtView1(0)("加入日"))) > acdt_date Then
                        y = i
                        FR_date = DateAdd(DateInterval.Year, i - 1, CDate(DtView1(0)("加入日")))
                        TO_date = DateAdd(DateInterval.Day, -1, DateAdd(DateInterval.Year, 1, CDate(FR_date)))
                        Exit For
                    End If
                Next

                '加入年度別処理
                Select Case Mid(qg_care_no, 1, 2)
                    Case Is = "A7"
                    ...
                    Case Is = "A8"
                        ...
                        use_amnt = 0
                        strSQL = "SELECT SUM(comp_eu_tech_chrg) AS comp_eu_tech_chrg, SUM(comp_eu_part_chrg) AS comp_eu_part_chrg"
                        strSQL += ", SUM(comp_eu_fee) AS comp_eu_fee, SUM(comp_eu_pstg) AS comp_eu_pstg"
                        strSQL += ", SUM(comp_eu_tax) AS comp_eu_tax"
                        strSQL += " FROM T01_REP_MTR"
                        strSQL += " WHERE (acdt_date >= CONVERT(DATETIME, '" & FR_date & "', 102)"
                        strSQL += " AND acdt_date <= CONVERT(DATETIME, '" & TO_date & "', 102))"
                        strSQL += " AND (qg_care_no = '" & qg_care_no & "')"
                        SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
                        DaList1.SelectCommand = SqlCmd1
                        DB_OPEN("nMVAR")
                        ...
                        If DtView2.Count <> 0 Then
                            If Not IsDBNull(DtView2(0)("comp_eu_tech_chrg")) Then
                                use_amnt = DtView2(0)("comp_eu_tech_chrg") + DtView2(0)("comp_eu_part_chrg") + DtView2(0)("comp_eu_fee") + DtView2(0)("comp_eu_pstg") + DtView2(0)("comp_eu_tax")
                            End If
                        End If
