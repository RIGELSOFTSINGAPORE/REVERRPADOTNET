...
#End Region
    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click
        ...
        If sfd.ShowDialog() = DialogResult.OK Then     'ダイアログを表示する
            ...
            'データを書き込む
            strData = "販売店名,加入証番号,加入者名,メーカー,型番,シリアル№,購入金額,保証種類,保証開始日,保証範囲,加入者請求金額,登録日付"
            swFile.WriteLine(strData)
            ...
            '合計
            strSQL = "SELECT V_M02_HSK.cls_code_name AS wrn_tem_name, V_M02_HSY.cls_code_name AS wrn_range_name, T01_member.wrn_fee, T01_member.wrn_range, COUNT(*) AS cnt, SUM(T01_member.wrn_fee) AS ttl"
            strSQL += " FROM T01_member INNER JOIN"
            strSQL += " T05_iPad ON T01_member.code = T05_iPad.code LEFT OUTER JOIN"
            strSQL += " V_M02_HSK ON T01_member.wrn_tem = V_M02_HSK.cls_code LEFT OUTER JOIN"
            strSQL += " V_M02_HSY ON T01_member.wrn_range = V_M02_HSY.cls_code"
            Select Case n
                ...
                Case Is = 3
                    ...
                    strSQL += str_WH(3)
            End Select
            strSQL += " GROUP BY V_M02_HSK.cls_code_name, V_M02_HSY.cls_code_name, T01_member.wrn_fee, T01_member.wrn_range"
            SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
            DaList1.SelectCommand = SqlCmd1
            DB_OPEN("nQGCare")
