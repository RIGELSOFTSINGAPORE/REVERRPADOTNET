...
#Region " Windows フォーム デザイナで生成されたコード "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.DataGridTableStyle1.GridColumnStyles.AddRange(New System.Windows.Forms.DataGridColumnStyle() {Me.DataGridBoolColumn1, Me.DataGridBoolColumn2, Me.DataGridTextBoxColumn1, Me.DataGridTextBoxColumn2, Me.DataGridTextBoxColumn3, Me.DataGridTextBoxColumn4, Me.DataGridTextBoxColumn5, Me.DataGridTextBoxColumn6, Me.DataGridTextBoxColumn7, Me.DataGridTextBoxColumn8, Me.DataGridTextBoxColumn9, Me.DataGridTextBoxColumn10, Me.DataGridTextBoxColumn11, Me.DataGridTextBoxColumn12, Me.DataGridTextBoxColumn13, Me.DataGridTextBoxColumn14, Me.DataGridTextBoxColumn15, Me.DataGridTextBoxColumn32})
        ...
        Me.DataGridTextBoxColumn32.HeaderText = "PCパック"
        Me.DataGridTextBoxColumn32.MappingName = "pc_pack"
        ...
        Me.DataGridTextBoxColumn33.HeaderText = "PCパック"
        Me.DataGridTextBoxColumn33.MappingName = "pc_pack"
        Me.DataGridTextBoxColumn33.NullText = ""
        Me.DataGridTextBoxColumn33.ReadOnly = True
        Me.DataGridTextBoxColumn33.Width = 70
        '
        'F00_Form20
        '
...
#End Region
        ...
        With OpenFileDialog1
            ...
            If .ShowDialog = DialogResult.OK Then
                ...
                For j = 17 To 65536
                    ...
                    'dtRow0("zip") = Trim(oSheet.Range("G" & j).Value)        '郵便番号
                    WK_str = Trim(oSheet.Range("H" & j).Value)
                    If WK_str <> Nothing Then WK_str = WK_str.Replace("'", "")
                    If WK_str <> Nothing Then WK_str = WK_str.Replace(vbCrLf, "")
                    dtRow0("adrs1") = MidB(StrConv(WK_str, VbStrConv.Narrow).Replace("'", ""), 1, 100)  '住所1
                    'dtRow0("adrs1") = MidB(Trim(WK_str).Replace("'", ""), 1, 100)  '住所1
                    WK_str = Trim(oSheet.Range("I" & j).Value)
                    If WK_str <> Nothing Then WK_str = WK_str.Replace(vbCrLf, "")
                    If WK_str <> Nothing Then WK_str = WK_str.Replace("'", "")
                    dtRow0("adrs2") = MidB(StrConv(WK_str, VbStrConv.Narrow).Replace("'", ""), 1, 100)  '住所2
                    'dtRow0("adrs2") = MidB(Trim(WK_str).Replace("'", ""), 1, 100)  '住所2 
                    dtRow0("tel") = MidB(StrConv(Trim(oSheet.Range("J" & j).Value), VbStrConv.Narrow), 1, 20)   '電話番号
                    'dtRow0("tel") = Trim(oSheet.Range("J" & j).Value)   '電話番号
                    WK_str = Trim(oSheet.Range("K" & j).Value)
                    If WK_str <> Nothing Then WK_str = WK_str.Replace("'", "")
                    dtRow0("vndr_name") = StrConv(WK_str, VbStrConv.Narrow)                             'メーカー名
                    'dtRow0("vndr_name") = Trim(WK_str)
                    ...
                    If WK_str <> Nothing Then WK_str = WK_str.Replace("'", "")
                    dtRow0("s_no") = MidB(WK_str, 1, 50)                                                'シリアル番号
                    dtRow0("prch_amnt") = Trim(oSheet.Range("N" & j).Value)                             '購入金額（税別）
                    dtRow0("wrn_tem") = Trim(oSheet.Range("O" & j).Value)                               '保証期間
                    dtRow0("makr_wrn_stat") = Trim(oSheet.Range("P" & j).Value)                         'メーカー保証開始
                    dtRow0("wrn_range") = Trim(oSheet.Range("Q" & j).Value)                             '保証範囲                    
                    dtRow0("pc_pack") = Trim(oSheet.Range("R" & j).Value)                               'PCパック申込者確認                    
                    '↑↑↑↑↑↑ 2021/10/15 修正
        ...
        If DtView1.Count <> 0 Then
            For k = 0 To DtView1.Count - 1
                ...
                If DG_Err_F = "0" Then
                    ...
                    dtRow1("souki") = 0
                    '↓↓↓↓↓↓ 2021/10/15 修正
                    dtRow1("pc_pack") = DtView1(k)("pc_pack")
                    '↑↑↑↑↑↑ 2021/10/15 修正
                ...
                Else
                    ...
                    '↓↓↓↓↓↓ 2021/10/15 修正
                    dtRow2("pc_pack") = DtView1(k)("pc_pack")
                    '↑↑↑↑↑↑ 2021/10/15 修正
    ...
    Sub DG_chk()
        ...
        If cmb01.Text <> Nothing Then
            ...
            strSQL = "SELECT T01_member.univ_code, T01_member.user_name"
            strSQL += " FROM T01_member LEFT OUTER JOIN"
            strSQL += " T05_iPad ON T01_member.code = T05_iPad.code"
            strSQL += " WHERE (T01_member.univ_code = " & cmb01.Text & ")"
            strSQL += " AND (T01_member.user_name = '" & DtView1(k)("name") & "')"
            strSQL += " AND (T01_member.nendo = " & P_proc_y & ") AND (T01_member.delt_flg = 0)"
            strSQL += " AND (T05_iPad.code IS NULL)"
            '↑↑↑↑↑↑ 2021/10/15 修正
            SqlCmd1 = New SqlClient.SqlCommand(strSQL, cnsqlclient)
            DaList1.SelectCommand = SqlCmd1
            '↓↓↓↓↓↓ 2021/10/15 修正
    ...
    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        ...
        If Err_F = "0" Then
            ...
            For j = 0 To WK_DtView1.Count - 1
                ...
                strSQL += ", Part_prt, Part_prt_bak, tonan_flg, zenson_flg, suisen_flg, souki_flg, pc_pack, imp_seq, memo, nendo, reg_date, delt_flg)"
                '↑↑↑↑↑↑ 2021/10/15 修正
                ...
                strSQL += ", 0"                                                                 '早期
                '↓↓↓↓↓↓ 2021/10/15 修正
                If WK_DtView1(j)("pc_pack") = "〇" Then
                    strSQL += ", 1"                                                             'PCパック申込者確認
                Else
                    strSQL += ", 0"                                                             'PCパック申込者確認
                End If
            ...
            For j = 0 To WK_DtView1.Count - 1
                ...
                strSQL += ", wrn_tem, makr_wrn_stat, wrn_range, pc_pack, Part_date, univ_code, shop_code"
                '↑↑↑↑↑↑ 2021/10/15 修正
                ...
                '↓↓↓↓↓↓ 2021/10/15 修正
                strSQL += ", '" & WK_DtView1(j)("pc_pack") & "'"                    'PCパック申込者確認
                '↑↑↑↑↑↑ 2021/10/15 修正
